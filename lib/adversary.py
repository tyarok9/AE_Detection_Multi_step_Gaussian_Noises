...
def deepfool_single(model, imgs, target, n_classes, train_mode, max_iter=50,
    ...
    if train_mode:
        ...
        model.eval()
    cpu_targets = target  # seems to be unnecessary
    imgs_original = torch.autograd.Variable(imgs)  # save original input for calculating perturbations
    imgs_var = imgs_original.clone()
    perturbations = torch.zeros(imgs_original.size()).cuda()
    criterion = torch.nn.NLLLoss()
    if is_gpu:
        criterion = criterion.cuda()
    for m in range(max_iter):
        imgs_var_in = imgs_var.expand(1, imgs_var.size(0), imgs_var.size(1),
                                      imgs_var.size(2))
        grad_input = imgs_var_in.repeat(n_classes, 1, 1, 1)
        output = model(imgs_var_in).clone()
        model.zero_grad()
        # calculates output, averaged loss and grad for each possible target class on the same image
        ...
        # update adversarial candidate
        imgs_var = imgs_var.add(ri)
        perturbations = perturbations.add(ri.data)  # seems to be unused
        imgs_var_in = imgs_var.clone().expand(1, imgs_var.size(0),
                                              imgs_var.size(1), imgs_var.size(2))
        # calculate output again to see if prediction changed
        output2 = model.forward(imgs_var_in).clone()
        _, pred2 = output2.data.cpu().max(1)
        pred2 = pred2.squeeze().item()
        # calculate perturbation - only for info/display purposes
        diff = torch.norm(imgs_original - imgs_var) / torch.norm(imgs_original)
        diff = diff.data.item()
        if verbose:
            print('iteration ' + str(m + 1) +
                  ': perturbation norm ratio = ' + str(diff))
        # stop if class changed
...
def attacking_mahalanobis_all(model, input, target, weight, loss_str, sample_mean, inv_sample_conv, ensemble_weight,
                              bound=0,
                              max_iter=100, step_size=0.01, kappa=0, p=2, drop_rate=0.0,
                              train_mode=False, verbose=False):
    is_gpu = next(model.parameters()).is_cuda
