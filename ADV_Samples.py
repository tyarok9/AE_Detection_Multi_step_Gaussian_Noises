...
parser.add_argument('--outf', default='./AEs/', help='folder to output results')
...
def main():
    ...
    for data, target in test_loader:
        ...
        for i in range(data.size(0)):
            if equal_flag[i] == 1:
                cleanclean_list.append(cleanclean_index)
            cleanclean_index += 1
        
    print(len(cleanclean_list))
    cleanclean_list = torch.LongTensor(cleanclean_list)
    clean_clean = torch.index_select(clean_data_tot, 0, cleanclean_list)
    clean_clean_label = torch.index_select(label_tot, 0, cleanclean_list)
    torch.save(clean_clean, '%s/clean_data_%s_%s_clean.pth' % (args.outf, args.net_type, args.dataset))
    torch.save(clean_clean_label, '%s/label_%s_%s_clean.pth' % (args.outf, args.net_type, args.dataset))
    torch.save(clean_clean, '%s/noisy_data_%s_%s_clean.pth' % (args.outf, args.net_type, args.dataset))
